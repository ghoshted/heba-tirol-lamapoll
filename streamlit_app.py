...
st.set_page_config(page_title="HeBA-Tirol · OSQ Reports", layout="centered")
...
def caption(text):
    st.markdown(f'<p class="tp-caption">{text}</p>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption(
        "This report summarises the HeBA/Tirol/OSQ survey: participant engagement, "
        "device usage, and response distributions. Data are sourced from the LamaPoll "
        "API and refreshed on each load."
    )
...
eyebrow("HEBA-TIROL")
display_heading("Reports")
today = datetime.date.today()
qualifier(f"updated {today.day:02d}·{today.month:02d}·{today.year % 100:02d}")
header_rule()
...
for i, (tab, (question_id, name)) in enumerate(zip(question_tabs, question_categories)):
    with tab:
        lamapoll_question_results_barchart(lama_api_key, poll_id, question_id, name, fig_number=4 + i)
